...
1.	**Device limitations with physical card readers** – Card readers are optimized for desktops or laptops, but more and more federal workforce use mobile devices. Derived PIV solutions may sometimes fill the gap but present cost and complexity challenges.
...
5.	**Modern technology doesn’t support PKI** - This challenge is decreasing as more software providers natively support PKI through native certificate-based authentication. Another perspective of this challenge is an agency's ability to leverage PIV credentials issued by other agencies. This could include a detailee or a collaboration. Some agency tools don't or can't support external PKI credentials.
...
Since FIDO2 leverages public key cryptography and can be configured as hardware-based, it is categorized as a **single-factor or multi-factor cryptographic device** authenticator type. It could achieve either AAL2 or AAL3, depending on the configuration and security control baseline.
