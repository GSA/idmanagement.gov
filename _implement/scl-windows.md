...
<div class="usa-alert usa-alert--error" role="alert">
  <div class="usa-alert__body">
    ...
    <p class="usa-alert__text">
      This technical implementation guide provides very detailed information on configuration of multiple components needed for enabling PIV network logon via an on-premise Active Directory Domain Controller.  Depending on your level of familiarity with some of these elements, you may benefit from a higher level summary of the pre-requisites and concepts provided in this <a class="usa-link" href="#" target="_blank" rel="noopener noreferrer">Summary Paper</a>.
    </p>
  </div>
</div>
...
<div class="usa-alert usa-alert--info" role="alert">
  <div class="usa-alert__body">
    ...
    <p class="usa-alert__text">
      If you are having issues with your on-premise Active Directory PIV logon after Microsoft’s full enforcement of strong identifiers in September 2025, see Step 4 below for guidance on resolution.
    </p>
  </div>
</div>
...
These Windows domain configuration guides will help you configure your on-premises Windows *network domain* for smart card logon using PIV credentials.
...
<div class="usa-alert usa-alert--info" role="alert">
  <div class="usa-alert__body">
    <h4 class="usa-alert__heading">Teamwork</h4>
    <p class="usa-alert__text">
      Work with your Network Engineers, Domain Admins, Account Management, and Information Security colleagues to review the information, perform the configurations, and troubleshoot any issues related to smart card logon for Windows domain.
    </p>
  </div>
</div>
...
3. Domain Controllers are running Microsoft Windows Server 2019 or newer to include September 2025 updates supporting Strong Certificate Binding.  
4. User workstations are joined to your network and are Windows 10 or newer.
...
1. [Network Ports and Protocols]({{site.baseurl}}/implement/scl-windows/#step-1---network-ports-and-protocols){:rel="noopener noreferrer"}{:class="usa-link"} includes specifics to ensure proper revocation checking is able to be performed  
2. [Domain Controllers]({{site.baseurl}}/implement/scl-windows/#step-2---domain-controllers){:rel="noopener noreferrer"}{:class="usa-link"} - assists in appropriate device credentialing  
3. [Trust Stores]({{site.baseurl}}/implement/scl-windows/#step-3---trust-stores){:rel="noopener noreferrer"}{:class="usa-link"} - provides guidance and resources on how to configure the NTauth and Enterprise trust stores to leverage enterprise PKI certificates  
4. [Account Linking: Associating PIV credentials with User Accounts]({{site.baseurl}}/implement/scl-windows/#step-4---account-linking){:rel="noopener noreferrer"}{:class="usa-link"} - includes information on strong certificate binding  
5. [Group Policies and Enforcement]({{site.baseurl}}/implement/scl-windows/#step-5---group-policies-and-enforcement){:rel="noopener noreferrer"}{:class="usa-link"} - assists with establishing enterprise wide configuration options to facilitate smartcard logon
...
The validation occurs in real time (with some caching) and requires ensuring network traffic is open and available to the destination web services, ports, and protocols including OCSP responses and Certificate Revocation Lists (CRLs). Many U.S. federal agencies implement a layered network security model with demilitarized zones (DMZs), proxies, and Trusted Internet Connections (TICs) to monitor, defend, and protect the networks, applications and users. In a Zero Trust Architecture (ZTA), these boundaries must not impede the verification of certificates.

## Verifying and Troubleshooting
Non-accessible endpoints for the web services due to firewalls blocking access is a very common root cause for errors. If you encounter user errors including “Cannot validate” and similar domain controller errors, your first troubleshooting step should be to verify your network and access. This is critical to maintaining the authenticator lifecycle integrity defined in the ILM Playbook.

<div class="usa-alert usa-alert--info" role="alert">
  <div class="usa-alert__body">
    <h4 class="usa-alert__heading">nslookup and certutil are your friendly tools</h4>
    <p class="usa-alert__text">
      Restricted or denied access to Internet web services including the OCSP and CRL web services used in the certificate validations lead to common errors and issues during the domain login process. Collaborate with your Network Engineers to review the web services, IP addresses, ports and protocols, and verify access from all local and wide-area network segments to ensure the certification path remains able to be valid.
    </p>
  </div>
</div>
...
Use certutil-url to specifically validate that the CRL Distribution Point (CDP) and Authority Information Access (AIA) locations are reachable from the client workstation within the Windows domain. If these paths are blocked, the Smart Card test will fail due to the system’s inability to verify the certificate’s status.

Certutil provides many options and two are covered here:

Export your public key and certificate for PIV Authentication to a .cer file (mypiv\_auth.cer), and run the following command in a command line from workstation(s) and domain controller(s):
...
<div class="usa-alert usa-alert--warning" role="alert">
  <div class="usa-alert__body">
    <h4 class="usa-alert__heading">Time is important</h4>
    <p class="usa-alert__text">
      When reviewing the verification messages, you should pay careful attention to the time. For example, if a CRL file is not downloaded in under 15 seconds, it is very likely that you will encounter network authentication errors and will need to perform some tuning. Ensuring these response times stay within acceptable thresholds may be vital to successfully completing revocation checking.
    </p>
  </div>
</div>
...
<div class="usa-alert usa-alert--error" role="alert">
  <div class="usa-alert__body">
    ...
    <p class="usa-alert__text">
      Yes. This very narrow class of services, that provide CRL and OCSP information for the purposes of verifying the revocation status of certificates used to make other HTTPS connections, should abide by best practices in the field and their respective specifications. For CRLs, follow <a class="usa-link" href="https://tools.ietf.org/html/rfc5280" target="_blank" rel="noopener noreferrer">RFC 5280</a>.which states CAs SHOULD NOT include URIs that specify https, ldaps, or similar schemes in extensions. Using port 80 for these services avoids a circular dependency where a secure connection cannot be validated because the TLS certificate used to negotiate the session key can only be validated by the data contained at the target destination. For OCSP, follow <a class="usa-link" href="https://tools.ietf.org/html/rfc6960" target="_blank" rel="noopener noreferrer">RFC 6960</a> which states a CA may use port 443 for OCSP where privacy is a requirement. 
      Note that CRLs and OCSP responses are digitally signed objects which can be independently validated by the consumer.  While connections may not be secured via TLS, the integrity of the contents cannot be compromised provided the signature is validated as coming from a trusted source.
      Agencies are encouraged to operate OCSP and CRL services via hostnames specifically reserved for those endpoints, so that other related information and functionality can be served securely and privately. For more information see the <a class="usa-link" href="https://https.cio.gov/guide/#are-federally-operated-certificate-revocation-services-crl-ocsp-also-required-to-move-to-https" target="_blank" rel="noopener noreferrer">Federal CIO Council HTTPS-Only Standard</a>. 
    </p>
  </div>
</div>
...
[Revocation]({{site.baseurl}}/university/pki/#revocation-checking){:rel="noopener noreferrer"}{:class="usa-link"}  status is validated using either Online Certificate Status Protocol (OCSP) or Certificate Revocation Lists (CRLs). To meet your initial network requirements, you should ensure the OCSP and CRL URLs included in *your agency* users’ [PIV Credential Certificates]({{site.baseurl}}/university/piv/#view-your-piv-credential-certificates){:rel="noopener noreferrer"}{:class="usa-link"} are accessible from all workstations and domain controllers.

In this context, the OCSP and CRL are the “services” being performed, while the URLs represent the specific “endpoints” where those services are hosted. Ensuring these endpoints are reachable confirms that the certification path is available for validation.
...
To enable communications with these Federal Common Policy Certificate Authority services, including those currently operational and any expansion, you should verify outbound communications to the base domain of *http.fpki.gov*. For example, a successful connection to [http://repo.fpki.gov/fcpca/fcpcag2.crt](http://repo.fpki.gov/fcpca/fcpcag2.crt){:target="_blank"}{:rel="noopener noreferrer"}{:class="usa-link usa-link--external"} will download a copy of the Federal Common Policy CA certificate. This root-level CA certificate is the foundation for the trust chain required to leverage PIV for authentication.

You should consider allowing two protocols (ports): HTTP (80) and DNS (53). Although the web services for publishing CRLs are not currently served over HTTPS (443), you may want to allow HTTPS (443) to future proof for any expansion. Ensuring these ports are open to the Federal PKI endpoints allows for the continuous revocation checking and path validation which are considered best-practice and meet the intent of RFC 5280 and NIST SP 800-53 authenticator management controls.
...
<div class="usa-alert usa-alert--info" role="alert">
  <div class="usa-alert__body">
    <h4 class="usa-alert__heading">Devices authenticate too!</h4>
    <p class="usa-alert__text">
...
* The certificate Subject Alternative Name must also contain the domain controller’s Global Unique Identifier (GUID) (i.e., for the “domain controller object”). This unique identifier links  the domain controller device identity to its public certificate.  
...
```
  ...
  * The domain controller’s certificate must be installed in the domain controller’s local computer’s *personal certificate store, in order to facilitate mutual authentication with a client*. 

<div class="usa-alert usa-alert--info" role="alert">
  <div class="usa-alert__body">
    <h4 class="usa-alert__heading">Supported Domain Controller Identifiers</h4>
    <p class="usa-alert__text">
     Many Domain Controllers are issued device certificates via Active Directory Certificate Services (ADCS) that may be operating an Only Locally Trusted PKI also known as an “Enterprise” PKI. Those ADCS implementers will want to include the Microsoft proprietary Security Identifier (SID) in their DC certificate profile to ensure compliance with recent Microsoft changes. For other AD implementers who receive DC certificates from their PIV Shared Service Provider, you may want to work with that provider to include the SID in any renewed or rekeyed DC certificates
    </p>
  </div>
</div>
...
<div class="usa-alert usa-alert--error" role="alert">
  <div class="usa-alert__body">
    ...
    <p class="usa-alert__text">
     Vulnerabilities identified in CVEs reported in May 2022 and reinforced by mandatory enforcement updates through 2025 have outlined potential avenues for network authentication based on spoofed or emulated device certificates that have not been issued by trusted CAs. In order to avoid these vulnerabilities, AD implementers must prohibit the use of any non-person entity (NPE or Devices to include DCs) certificates issued by CAs listed in GPO policy tuples. Please see the following <a class="usa-link usa-link--external" href="https://support.microsoft.com/en-us/topic/kb5014754-certificate-based-authentication-changes-on-windows-domain-controllers-ad2c23b0-15d8-4340-a468-4d4f3b188f16" target="_blank">Microsoft Knowledge Blog</a> for additional specifics regarding these vulnerabilities.
     Inclusion of the “strong” SID into auto-enrolled ADCS device certificates prevents illegitimate devices from emulating a legitimate certificate.
    </p>
  </div>
</div>
...
If you do have a local enterprise CA, [here are some tips](#step-7---local-certificate-authority){:class="usa-link"}. Note that after September 2025 Windows server patches, Domain Controllers should be configured for auto-enrollment in ADCS to ensure SID extension is included and populated in the resulting device certificate.
...
<div class="usa-alert usa-alert--info" role="alert">
  <div class="usa-alert__body">
    <h4 class="usa-alert__heading">Mutual Trust</h4>
    <p class="usa-alert__text">
     Note that both the client machine and domain controller trust stores must be configured for mutual authentication. As a result, the domain controller issuing and root CA certificates need to be included in the client trust store and the client PIV authentication issuing, intermediate and root certificates need to be able to be trusted by the domain controller. Establishing this bidirectional trust is foundational to the PKINIT authentication protocol.
    </p>
  </div>
</div>
...
* A policy tuple will need to be configured in your GPO settings to continue to leverage UPN, see example 1 under Step 4 below.
...
<div class="usa-alert usa-alert--warning" role="alert">
  <div class="usa-alert__body">
    <h4 class="usa-alert__heading">Use of UPN by Applications</h4>
    <p class="usa-alert__text">
     You may find that you have many applications that rely on User Principal Name values. There is no need to remove existing or stop populating new User Principal Name values in your transition to altSecurityIdentities. Maintaining these values allows legacy applications to continue to function while network authentication  is upgraded to the more flexible altSecurityIdentities mapping 
    </p>
  </div>
</div>
...
<div class="usa-alert usa-alert--warning" role="alert">
  <div class="usa-alert__body">
    <h4 class="usa-alert__heading">Deprecated Mapping</h4>
    <p class="usa-alert__text">
     As of September 2024, Microsoft has deprecated the use of Subject and RFC822 altsecid mappings as they were considered vulnerable to spoofing attempts. In this context, removing these "weak" identifier mapping ensures that the user account is protected from unauthorized access caused by certificate impersonation.
    </p>
  </div>
</div>
...
| Subject Key Identifier | X509:\<br/> | X509:\<SKI>df2f4b04462a5aba81fec3a42e3b94beb8f2e087 | Supported | Highly unique; may be difficult to manage. |
...
| Subject | X509:\<S> | X509: \<S>C=US,O=U.S. Government, OU=Government Agency, CN=JANE DOE OID.0.9.2342.19200300.100.1.1=25001003151020 | Deprecated | This field is no longer supported for altsecid mapping as of Sept 2024 and as of September 2025 will be rejected in Full Enforcement. |
| RFC822 name | X509:\<RFC822> | X509: \<RFC822>john.smith@hhs.gov | Deprecated | This field is no longer supported for altsecid mapping as of Sept 2024 and as of September 2025 will be rejected in Full Enforcement. |

<div class="usa-alert usa-alert--info" role="alert">
  <div class="usa-alert__body">
    <h4 class="usa-alert__heading">Use of Security Identifiers</h4>
    <p class="usa-alert__text">
     Although it is not mandated by FPKI PIV certificate profiles, an SID is a Microsoft proprietary, but publicly documented, identifier that can be asserted as a non-critical extension in a PIV authentication certificate and used for AD user account mapping.
    </p>
  </div>
</div>

Policy tuple mappings to accommodate some weak identifier mappings are available in Windows Server 2019 and later as of September 10th, 2024\. Tuple mappings are defined via group policy, specifically *Administrative Settings / System / KDC* under an entry titled “Allow name-based strong mappings for certificates” wherein administrators can define three things to include:

1. Trusted issuing CA - identified by its certificate thumbprint, each tuple will be unique to each issuing CA thumbprint  
2. Trusted certificate policy OID - ensures that the client certificate is issued according to a certain policy or policies, and  
...
2.   
   Altsecid policy tuple for a privileged administrator with an ALT token (Entrust Managed Services SSP CA, Common-Hardware Token, Issuer/Subject altsecid)
...
3.   
   Altsecid policy tuple for an external PIV or dPIV user (NASA Operational CA, PIV Authentication and dPIV Authentication OIDs, Issuer/Subject altsecid)

```
     67ddd6f4be3b69568f591bf999db2ef3085f7c5bl, 2.16.840.1.101.3.2.1.3.13, 2.16.840.1.101.3.2.1.3.40; IssuerSubject
```

If you are not already using “strong identifiers,” explicitly defining these tuples will help mitigate the threats associated with certificate spoofing which is the aim of the February 2025 Microsoft "Strong Mapping" enforcement.

Identity certificates used for Windows logon can generally be found:

* On the smart card itself, or on the client’s local trust store.  
...
To gather the certificate from the smart card using a Windows workstation, have the cardholder do the following below:

This procedure allows the user to extract the public certificate required for account linking.
...
6. Have the cardholder select “Click here to view certificate properties,” under the validity date range. The appropriate certificate will list “Smart Card Logon” in the intended purposes on the General tab. If the certificate has this purpose listed, have the cardholder proceed to <a class="usa-link usa-link--external" href="https://www.idmanagement.gov/implement/scl-windows/#step-7---local-certification-authority" target="_blank">Step 7</a>. Otherwise, have the cardholder close the certificate, click more choices on the Certificate List, click another certificate in the list, and click Click here to view certificate properties until the correct certificate has been identified. 
...
<div class="usa-alert usa-alert--info" role="alert">
  <div class="usa-alert__body">
    <h4 class="usa-alert__heading">PIV Authentication Certificate Sources</h4>
    <p class="usa-alert__text">
     USAccess customers can receive PIV authentication certificates via their SIP interface. Reach out to USaccess at GSA.gov for additional information.
    </p>
  </div>
</div>
...
This setting tells your network domain *I don’t always want to use the Subject Alternate Name values for my user certificates.* More information on the setting is available [here.](https://technet.microsoft.com/en-us/library/ff520074\(WS.10\).aspx){:target="_blank"}{:rel="noopener noreferrer"}{:class="usa-link usa-link--external"}
...
<div class="usa-alert usa-alert--warning">
  <div class="usa-alert__body">
    <h4 class="usa-alert__heading">User Based Enforcement</h4>
    <p class="usa-alert__text">
     The user’s password will no longer be known by the user. Look for agency internal applications that are still using username and password and performing Form Based Authentication against the network directories. Fix these using Kerberos, SAML, or direct x509 authentication.
    </p>
  </div>
</div>
...
The *Number of previous logons to cache* can be modified in local or group policy in the following location *Computer Configuration\Windows Settings\Security Settings\Local Policies\Security options*
...
<div class="usa-alert usa-alert--error">
  <div class="usa-alert__body">
    <h4 class="usa-alert__heading">Can federally operated certificate revocation services (CRL, OCSP) operate on port 80?</h4>
    <p class="usa-alert__text">
     Yes. This very narrow class of services, that provide CRL and OCSP information for the purposes of verifying the revocation status of certificates used to make other HTTPS connections, should abide by best practices in the field and their respective specifications. For CRLs, follow <a class="usa-link" href="https://tools.ietf.org/html/rfc5280" target="_blank" rel="noopener noreferrer">RFC 5280</a> which states CAs SHOULD NOT include URIs that specify https, ldaps, or similar schemes in extensions. For OCSP, follow <a class="usa-link" href="https://tools.ietf.org/html/rfc6960" target="_blank" rel="noopener noreferrer">RFC 6960</a> which states a CA may use port 443 for OCSP where privacy is a requirement. Agencies are encouraged to operate OCSP and CRL services via hostnames specifically reserved for those services, so that other related information and functionality can be served securely and privately. For more information see the <a class="usa-link" href="https://https.cio.gov/guide/#are-federally-operated-certificate-revocation-services-crl-ocsp-also-required-to-move-to-https" target="_blank" rel="noopener noreferrer">Federal CIO Council HTTPS-Only Standard</a>. 
    </p>
  </div>
</div>
...
4. Configure Certificate Services Client – Auto-Enrollment with the following options:  
   ...
   *Renew Expired Certificates, Update Pending Certificates, Remove Revoked Certificates*: *Check\_all checkboxes*.  
...
To grant a user access, based on the type of authenticator used, you can use a Windows Active Directory (AD) feature called *Authentication Mechanism Assurance (AMA)*. AMA allows you to add a group membership identifier to the user’s Kerberos token.
...
<div class="usa-alert usa-alert--warning">
  <div class="usa-alert__body">
    ...
    <p class="usa-alert__text">
      ...
      <a class="usa-link" href="https://www.idmanagement.gov/playbooks/pam/" target="_blank" rel="noopener noreferrer">Privileged Identity Playbook</a>
      for best practices in reducing risk associated with privileged accounts and access. This separation of duties is a core component of privilege management in NIST SP 800-53-5.
    </p>
  </div>
</div>
...
3. The user’s session is granted a Kerberos ticket that includes the additional group membership  
4. The user browses to the other federal agency’s application  
5. The user’s browser is redirected to his/her home agency’s Federation Service  
6. The Federation Service at the home agency finds the Kerberos ticket for the user’s session  
7. A Security Assertion Markup Language (SAML) assertion is created by the Federation Service (This is a token translation.)  
8. The SAML assertion includes the AD group membership information that identifies that this user authenticated with a PIV credential  
9. The user’s browser is redirected back to the other federal agency’s application  
10. The user is successfully authenticated with the valid SAML assertion  
11. The other federal agency’s application is configured to allow access to only those users who have authenticated using a PIV credential
...
2. The certificate policy OID asserted allows Microsoft AD on the home agency’s network to assign the user to a group specifically for PIV authenticated users  
3. The user’s session is granted a Kerberos ticket that includes the additional group membership  
4. The SharePoint site is configured to only allow access to only those users who have authenticated using a PIV credential
 
## Other Considerations and References

Use the Windows Registry Editor to set the *AMA Priority* above *Most Recently Issued Superior Certificate Heuristic*:
...
<div class="usa-accordion usa-accordion--bordered">
  ...
  <div id="m-a2" class="usa-accordion__content usa-prose">
    ...
    <ol type="1">
      ...
      <li>Type cmd.</li>
      <li>Click Command Prompt, shown under Best Match.</li>
      ...
      <li>In the command prompt, type certutil -scinfo and press Enter.</li>
      ...
      <li>A functioning card will return information on the card type and reader, begin polling for keys and validating certificates, and prompt for PIN entry. If this is the case with the card you are testing, click Cancel and close out of the command prompt.</li>
...
<img src="{{site.baseurl}}/assets/piv/pivauth-command-prompt-done.png" alt="A screenshot of a command prompt window that includes the word done near the bottom of the window." width="593" height="474">
  ...
  <div id="m-a3" class="usa-accordion__content usa-prose">
    ...
    <hr />
    <h2>Symptom</h2>
    <p>After PIN entry, the following error is displayed on the logon screen: <br>Signing in with a smart card isn’t supported for your account. For more information, contact your administrator.</p>
    ...
    <ol type="1">
      <li>Log in to Windows using a password.</li>
      <li>Open the Start Menu, located in the bottom left corner of the screen.</li>
      <li>Type event viewer.</li>
      <li>Click Event Viewer, shown under Best Match.</li>
      ...
      <li>Click Operational.</li>
      <li>On the right side of the window, under Actions, click Enable Log (skip this step if the option reads ”Disable Log”; the log is already enabled).</li>
    ...
    <ol type="1">
      ...
      <li>If prompted by a User Account Control pop-up, click Yes.</li>
      ...
      <li>On the left side of the MMC window, click the > symbol to expand these items on the tree:</li>
      <ol type="a">
  ...
  <div id="m-a4" class="usa-accordion__content usa-prose">
    ...
    <ol type="1">
      <li>An untrusted certification authority was detected while processing the smart card certificate used for authentication.</li>
      ...
      <li>The smart card used for authentication has been revoked.</li>
    ...
    <ol type="1">
      ...
      <li>Click Command Prompt, shown under Best Match.</li>
      <img src="{{site.baseurl}}/assets/piv/pivauth-best-match-command-prompt.png" alt="A screenshot of the Command Prompt app icon. The words Best Match appear above the icon.">
      <li>In the command prompt, type echo %logonserver% and press Enter.</li>
    ...
    <p>On the domain controller indicated above:</p>
    <ol type="1">
      <li>Log in as a Domain Administrator.</li>
      <li>Open the Start Menu.</li>
      <li>Type mmc.exe.</li>
      ...
      <li>On the left side of the Event View, click the > symbol to expand each of these items on the tree:</li>
      <ol type="a">
      ...
      <li>On the domain controller, still in Event Viewer, on the right pane, click Refresh.</li>
      <li>New log events will be shown. Look for the events with an “Error” status  and the task category “Build Chain.”</li>
      <li>Click the Details tab. In the UserData section, look for the user’s name in the Certificate [subjectName] field. If you do not see the user’s name, continue scrolling through the list of events to find the next event with an “Error” status and the task category “Build Chain.” Using results filtering may help to narrow this list down.</li>
    ...
    <br>
    <p>Example 1: A certificate in the path is revoked.</p>
    ...
    <p>Example 2: The path does not build to a trust anchor</p>
    ...
    <p>Example 3: The revocation status is unreachable, or the revocation status signature cannot be validated due to an invalid trust path.</p>
...
<p>You can also use the <a class="usa-link usa-link--external" href="http://pkif.sourceforge.net/pitt.html" target="_blank" rel="noopener noreferrer">PKI Interoperability Test Tool (PITT)</a>, listed on the <a class="usa-link" href="{{site.baseurl}}/implement/#fpki-troubleshooting-tools" target="_blank" rel="noopener noreferrer">Useful Tools page</a>, to validate the certificate path on the logon server. The <a class="usa-link usa-link--external" href="http://pkif.sourceforge.net/pitt_usage.pdf" target="_blank" rel="noopener noreferrer">PITT Usage Guide</a> contains procedures for using the tool.</p>    
    ...
    <p>Follow the steps in the <a class="usa-link" href="{{site.baseurl}}/implement/scl-windows/#step-3---trust-stores " target="_blank" rel="noopener noreferrer">Trust Stores Playbook</a> to add the appropriate issuing CA for the PIV card to the Enterprise NTAuth trust store.</p>
     <hr />    
    <h2>Symptom</h2>
    <p>During smart card logon attempt, the following error is displayed on the logon screen:  <br>The system could not log you on. Your credentials could not be verified.</p>
  <hr />
